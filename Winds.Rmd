...
### take only the tps msw (maximum sustained winds) layers 
Cam_tps_msw <- app(winds_Camille$tps[[grepl("msw",names(winds_Camille$tps))]],fun="max",na.rm=TRUE)
terra::plot(project(Cam_tps_msw,"epsg:4326"),main="Hurricane Camille, Maximum Sustained Winds (m/s) TPS",plg = list(title = "(m/s)"),range=c(0,80),title.cex=0.8)
...
terra::panel(project(Sin_power,"epsg:4326"),main="Typhoon Sinlaku, Total Maximum Potential Wind Power",plg = list(title = "Power (kWh)"),title.cex=0.8,range=c(0,16000),fun=function() lines(vect(OC)))
```
...
terra::panel(project(Mar_dur,"epsg:4326"),main="Hurricane Maria, Cat 1+ Wind Duration",plg = list(title = "hrs"),range=c(0,16),fun=function() lines(vect(PR)),title.cex=0.8)
```
