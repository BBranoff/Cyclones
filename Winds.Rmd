...
Wind related products are provided in a mix of both imperial (knots) and metric 
(m/s) units. This is somewhat by design. The baseline data for all Cyclones workflows are the
tabular IBTrACS or HURDAT, both of which originate at the US National Hurricane
Center, which uses imperial units. Thus, to preserve the native values, storm track
products are provided in both knots and m/s. Downstream products, however switch
to metric as they are already undergoing approximations and transformations. 

* [Get Storms](#get-storm-tabular-data)
* [Get Wind Extents](#make-wind-extent-spatial-features)
  * [Build Extent Models](#build-extent-models)
  * [Make Extents](#make-wind-extent-spatial-features)
* [Get Winds](#build-wind-rasters)
  * [Wind Power and Direction](#wind-power-and-direction)
* [Validate Winds](#compare-winds-with-any-corresponding-drop-sondes)
* [Aggregate Winds](#aggregate-winds )
...
argument, which can be one of: USA, WMO, TOKYO, CMA, HKO, KMA, NEWDELHI, REUNION, BOM,
...
windextent_1 <- make_extents(storms$CAMILLE_1969_NA_1969226N18280,
                             mods=mods,type=c("linestrings","polygons"))
...
Cam2 <- ggplot(windextents[[1]]|>filter(kts>=64|location=="eye",source %in% c("native","native & modeled","interpolated & native","interpolated & modeled"))|>
...
winds_df <- lapply(grep("CAMILLE|MARIA|SINLAKU",list.dirs("I:/Cyclone Products/Wind/v3_modeled/",recursive=TRUE,full.names=TRUE),value=TRUE),function(x){
  if (grepl("boose|holland|tps|willoughby",x)&grepl("5000_30",x)){
    ...
    names(df)[3:length(names(df))] <-
      paste(names(df)[3:length(names(df))],rep(unique(time(r)),each=3),sep="_")
    df <- df |>
...
Cam=ggplot(data=winds_df|>filter(storm=="Camille",unit=="msw.ms")) +
...
Sin=ggplot(data=winds_df|>filter(storm=="Sinlaku",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(method~time)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Maximum\nSustained\nWinds\n(m/s)",
...
Mar=ggplot(data=winds_df|>filter(storm=="Maria",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(method~time)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Maximum\nSustained\nWinds\n(m/s)",
                       limits = c(0, 80),breaks=c(0,20,40,60,80))+ ##  same as Camille because they will share the same legend
...
winds_df_unmodeled <- lapply(grep("CAMILLE|MARIA|SINLAKU",list.dirs("I:/Cyclone Products/Wind/v3_unmodeled/",recursive=TRUE,full.names=TRUE),value=TRUE),function(x){
  if (grepl("boose|holland|tps|willoughby",x)&grepl("5000_30",x)){
    ...
    #r <- r[[grep("msw",names(r))]]
    df <- as.data.frame(project(r,"epsg:4326"),xy=TRUE)
    names(df)[3:length(names(df))] <- paste(names(df)[3:length(names(df))],rep(unique(time(r)),each=3),sep="_")
    df <- df |>
      tidyr::pivot_longer(cols=-c(x,y),names_sep = "_",names_to = c("method","unit","time"))|>
      mutate(storm=storm)
  }
...
Sin <- ggplot(data=winds_df_unmodeled |>filter(storm=="Sinlaku",time=="2026-04-14 03:30:00",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
...
Cam <- ggplot(data=winds_df_unmodeled |>filter(storm=="Camille",time=="1969-08-18 03:00:00",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(source~method)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Maximum\nSustained\nWinds\n(m/s)",
                       limits = c(0, 80))+
...
Mar <- ggplot(data=winds_df_unmodeled |>filter(storm=="Maria",time=="2017-09-20 09:00:00",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(source~method)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Maximum\nSustained\nWinds\n(m/s)",
                       limits = c(0, 80))+
...
all <- ggplot(data=winds_df_unmodeled|>filter(storm=="Sinlaku",time=="2026-04-14 03:30:00",source=="modeled",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(source~method)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Maximum\nSustained\nWinds\n(m/s)",
...
zoom <- ggplot(data=winds_df_unmodeled|>filter(storm=="Sinlaku",time=="2026-04-14 03:30:00",unit=="msw.ms")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(source~method)+
...
```{r plotwinds4,echo=FALSE,message=FALSE,warning=FALSE,fig.width=9,fig.height=6,fig.cap="Fig 6. Wind direction and power are also provided by 'get_wind()'. Both variables are calculated the same across the different methods, except for Boose, which incporporates a cross-isobar inflow angle to get direction. Direction is usefull when considering wind effects across topology and can be used alongside an elevation dataset to calculate an exposure index. Power is mostly proportional to the velocity cubed and is important because it considers how long winds were present in a given area, which can also have an impact on the overall effect of the wind."}
Sin_d <- ggplot(data=winds_df_unmodeled |>filter(storm=="Sinlaku",time=="2026-04-14 03:30:00",unit=="windir.deg",source=="modeled")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(time~method)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Wind Direction (deg)",
                       option="magma")+
  theme_bw()+
  geom_sf(data=rnaturalearth::ne_countries(scale=10,continent="Oceania"),fill=NA,col="black")+
  ggtitle("Typhoon Sinlaku Wind Direction")+
  scale_x_continuous(limits=c(137,155),n.breaks=4)+
  ylim(7,26)+
  theme(axis.title = element_blank(),panel.grid=element_blank(),legend.position="right")
Sin_p <- ggplot(data=winds_df_unmodeled |>filter(storm=="Sinlaku",time=="2026-04-14 03:30:00",unit=="power.kWh",source=="modeled")) +
  geom_raster(aes(x=x,y=y,fill=value))+
  facet_grid(time~method)+
  scale_fill_viridis_c(n=10,na.value = "transparent",name="Wind Power (kWh)",
                       option="magma")+
  theme_bw()+
  geom_sf(data=rnaturalearth::ne_countries(scale=10,continent="Oceania"),fill=NA,col="grey")+
  ggtitle("Typhoon Sinlaku Wind Power")+
  scale_x_continuous(limits=c(137,155),n.breaks=4)+
  ylim(7,26)+
  theme(axis.title = element_blank(),panel.grid=element_blank(),legend.position="right")
...
ggdirall<- ggplot(comps_r_filt|>filter(var=="windir.deg")|>mutate(storm=factor(storm))) +
  ...
  geom_text(data=comps_r_filt|>filter(storm %in% storms)|>
              ...
              summarise(RMSE=paste0("RMSE:",round(sqrt(mean((value-WD)^2,na.rm=TRUE)),1),
                        ...
                        WD=100,value=340)|>ungroup(),aes(x=WD,y=value,label=RMSE,hjust=0))+
  facet_grid(~method)+
  guides(col=guide_legend("Saffir-Simpson\nCategory"))+
  theme_bw()+
  theme(legend.position = "bottom")
...
```{r plotwinds_agg_false,eval=FALSE,echo=TRUE}
...
```{r plotwinds_agg1,eval=TRUE,echo=FALSE,message=FALSE,warning=FALSE,results='hide',fig.width=7,fig.height=4,fig.align='left'}
winds_Camille <- get_wind("CAMILLE1969_5000_30_WIND",todir="I:/Cyclone Products/Wind/v3_modeled/",loadrasts=TRUE)
### take only the tps msw (maximum sustained winds) layers 
Cam_tps_msw <- app(winds_Camille$tps[[grepl("msw",names(winds_Camille$tps))]],fun="max",na.rm=TRUE)
<<<<<<< HEAD
terra::plot(project(Cam_tps_msw,"epsg:4326"),main="Hurricane Camille Maximum Sustained Winds (m/s) TPS",plg = list(title = "(m/s)"),range=c(0,80),title.cex = 1.1 )
=======
terra::plot(project(Cam_tps_msw,"epsg:4326"),main="Hurricane Camille, Maximum Sustained Winds (m/s) TPS",plg = list(title = "(m/s)"),range=c(0,80),cex.main=0.9)
>>>>>>> 40331876e7170ec1cb10f5950b2b093e3500671e
terra::plot(st_geometry(rnaturalearth::ne_countries()),add=TRUE)
```
```{r plotwinds_agg2,eval=TRUE,echo=FALSE,message=FALSE,warning=FALSE,results='hide',fig.width=7,fig.height=5,fig.align='left'}
##  Additionally, we could take the sum of the total potential wind power 
winds_Sinlaku <- get_wind("Sinlaku2026_5000_30_WIND",todir="I:/Cyclone Products/Wind/v3_modeled/",loadrasts=TRUE)
Sin_power <- rast(lapply(winds_Sinlaku,function(x) x[[grep("power",names(x))]]))
methods <- unlist(sapply(strsplit(names(Sin_power),"_"),"[[",1))
Sin_power <- terra::tapp(Sin_power,index=methods,fun="sum",na.rm=TRUE)
OC <- st_geometry(rnaturalearth::ne_countries(continent="Oceania",scale=10))
terra::panel(project(Sin_power,"epsg:4326"),plg = list(title = "Power (kWh)"),range=c(0,16000),fun=function() lines(vect(OC)))
```
```{r plotwinds_agg3,eval=TRUE,echo=FALSE,message=FALSE,warning=FALSE,results='hide',fig.width=7,fig.height=3,fig.align='left',fig.cap="Fig 9. Aggregating the individual time step layers from get_wind() produces a whole-storm perspective. In this case, the maximum wind (top) as well as the total power (middle) and the duration of hurricane force winds (bottom). Aggregating can be done in many complicated ways and a dedicated agg_winds() function is forthcoming."}
###  We could also seek the duration of hurricane force winds
###  For now, this is a custom function but future methods will include dedicated methods
###  such as 'aggregate_winds()'
winds_Maria <- get_wind("Maria2017_5000_30_WIND",todir="I:/Cyclone Products/Wind/v3_modeled/",loadrasts=TRUE)
...
Mar_dur <- terra::tapp(rast(Mar_dur),index=methods,fun="sum",na.rm=TRUE)
#Mar_durMaj <- terra::tapp(rast(Mar_durMaj),index=methods,fun="sum",na.rm=TRUE)
PR <- st_geometry(rnaturalearth::ne_countries(country="puerto rico",scale=10))
terra::panel(project(Mar_dur,"epsg:4326"),plg = list(title = "hrs"),range=c(0,16),fun=function() lines(vect(PR)))
```
